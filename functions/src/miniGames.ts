...
// 3-Card Poker Evaluator
function evaluate3CardHand(cards: Card[]): number {
    const sorted = [...cards].sort((a, b) => b.value - a.value);
    const v1 = sorted[0].value, v2 = sorted[1].value, v3 = sorted[2].value;
    
    const isFlush = (sorted[0].suit === sorted[1].suit) && (sorted[1].suit === sorted[2].suit);
    const isSpecialStraight = (v1 === 14 && v2 === 3 && v3 === 2);
    const isNormalStraight = (v1 - 1 === v2) && (v2 - 1 === v3);
    const isStraight = isNormalStraight || isSpecialStraight;
    const is3OfKind = (v1 === v2) && (v2 === v3);
    ...
    if (isStraight && isFlush) {
        typeScore = 50000000;
        highCardScore = isSpecialStraight ? 3 : v1;
    } else if (is3OfKind) {
        typeScore = 40000000;
        highCardScore = v1;
    } else if (isStraight) {
        ...
        typeScore = 20000000;
        highCardScore = (v1 << 8) | (v2 << 4) | v3;
    } else if (isPair) {
        typeScore = 10000000;
        const pairVal = (v1 === v2) ? v1 : v2;
        const kicker = (v1 === v2) ? v3 : v1;
        highCardScore = (pairVal << 4) | kicker;
    } else {
        typeScore = 0;
        highCardScore = (v1 << 8) | (v2 << 4) | v3;
    }
    
    return typeScore + highCardScore;
}

const NORNS_DURATIONS = {
    betting: 20000,
    reveal: 6000,
    result: 4000
};

export const placeNornsFateBet = onCall(
    { cors: true, maxInstances: 10 },
    async (request) => {
        if (!request.auth) throw new HttpsError('unauthenticated', 'Login required');
        const { uid } = request.auth;
        const { handId, amount } = request.data;

        if (!['odin', 'thor', 'loki'].includes(handId) || !amount || amount <= 0) {
            throw new HttpsError('invalid-argument', 'Invalid bet');
        }

        const db = admin.firestore();
        const userRef = db.collection('users').doc(uid);
        const stateRef = db.collection('settings').doc('mini_games').collection('norns_fate').doc('current');
        ...
        try {
            const result = await db.runTransaction(async (transaction: any) => {
                ...
                const phaseDuration = now - (state?.lastUpdate || 0);
                const isBetting = state?.phase === 'betting';
                const isGracePeriod = state?.phase === 'reveal' && phaseDuration < 1500;

                if (!state || (!isBetting && !isGracePeriod)) throw new Error('Betting is closed for this draw.');

                const userData = userSnap.data() || {};
                const currentPoints = userData.points || 0;
                if (currentPoints < amount) throw new Error('Insufficient Valcoins');

                const existingBet = betSnap.exists ? betSnap.data() || {} : {};
                const currentTotal = existingBet.total || 0;
                if (currentTotal + amount > MAX_BET_PER_USER) throw new Error(`Max bet per deal is ${MAX_BET_PER_USER}.`);

                const currentHandPool = (state.pools && state.pools[handId]) ? state.pools[handId] : 0;
                if (currentHandPool + amount > MAX_SHIP_POOL) throw new Error('This hand pool is full!');
                ...
                transaction.update(stateRef, {
                    [`pools.${handId}`]: admin.firestore.FieldValue.increment(amount)
                });

                if (betSnap.exists) {
                    transaction.update(betRef, {
                        [handId]: admin.firestore.FieldValue.increment(amount),
                        total: admin.firestore.FieldValue.increment(amount)
                    });
                } else {
                    transaction.set(betRef, {
                ...
                return { success: true, newBalance: currentPoints - amount, playerName: userData.auroryPlayerName || userData.displayName || 'Guest', playerAvatar: userData.auroryProfilePicture || userData.photoURL || '' };
            });

            const rtdb = admin.database();
            const resultData = result as any;
            await rtdb.ref(`norns_fate/bettors/${handId}/${uid}`).set({
                name: resultData.playerName,
                avatar: resultData.playerAvatar
            });
...
export const refreshNornsFate = onCall(
    { cors: true, maxInstances: 5 },
    async (request) => {
        ...
        try {
            const result = await db.runTransaction(async (transaction: any) => {
                ...
                if (!state) state = { phase: 'result', endTime: now - 1, raceId: 0 };
                
                // If the time hasn't passed yet, we don't change the phase
                if (now < state.endTime) return { state, changed: false };
                ...
                if (state.phase === 'betting') {
                    nextPhase = 'reveal';
                    duration = NORNS_DURATIONS.reveal;
                    
                    const odinScore = evaluate3CardHand(state.hands.odin);
                    const thorScore = evaluate3CardHand(state.hands.thor);
                    const lokiScore = evaluate3CardHand(state.hands.loki);
                    
                    const scores = [odinScore, thorScore, lokiScore];
                    const maxScore = Math.max(...scores);
                    let winnerHand = 'odin';
                    if (thorScore === maxScore) winnerHand = 'thor';
                    if (lokiScore === maxScore) winnerHand = 'loki';

                    updates = { winnerHand, scores };

                } else if (state.phase === 'reveal') {
                    nextPhase = 'result';
                    duration = NORNS_DURATIONS.result;
                } else {
                    nextPhase = 'betting';
                    duration = NORNS_DURATIONS.betting;
                    
                    const configRef = db.collection('settings').doc('mini_games');
                    const configSnap = await transaction.get(configRef);
                    const config = configSnap.exists ? configSnap.data()?.nornsFate || {} : {};
                    const houseSeed = config.houseSeed ?? DEFAULT_HOUSE_SEED;

                    let generatedState = false;
                    let odinHand, thorHand, lokiHand;

                    while(!generatedState) {
                        const deck = generateDeck();
                        odinHand = [deck[0], deck[1], deck[2]];
                        thorHand = [deck[3], deck[4], deck[5]];
                        lokiHand = [deck[6], deck[7], deck[8]];
                        
                        const faceRanks = [
                            {id: 'odin', val: odinHand[0].value}, 
                            {id: 'thor', val: thorHand[0].value}, 
                            {id: 'loki', val: lokiHand[0].value}
                        ].sort((a,b) => b.val - a.val);

                        const roll = Math.random();
                        let targetWinner = '';
                        if (roll < 0.34) {
                            targetWinner = faceRanks[2].id;
                        } else if (roll < 0.67) {
                            targetWinner = faceRanks[1].id;
                        } else {
                            targetWinner = faceRanks[0].id;
                        }

                        const oFinal = evaluate3CardHand(odinHand);
                        const tFinal = evaluate3CardHand(thorHand);
                        const lFinal = evaluate3CardHand(lokiHand);

                        let actualWinner = 'odin';
                        let maxFinal = oFinal;
                        if (tFinal > maxFinal) { maxFinal = tFinal; actualWinner = 'thor'; }
                        if (lFinal > maxFinal) { maxFinal = lFinal; actualWinner = 'loki'; }
                    ...
                    updates = {
                        raceId: (state.raceId || 0) + 1,
                        hands: { odin: odinHand, thor: thorHand, loki: lokiHand },
                        winnerHand: null,
                        pools: { odin: houseSeed, thor: houseSeed, loki: houseSeed },
                        houseSeed,
                        multiplierFactor: config.multiplierFactor ?? (1 - DEFAULT_HOUSE_CUT)
                    };
                    
                    if (config.botsEnabled !== false) {
                       const botActions: any[] = [];
                       const botPools: Record<string, number> = { odin: 0, thor: 0, loki: 0 };
                       const shipIds = ['odin', 'thor', 'loki'];
                       const botCount = Math.floor(Math.random() * 11) + 10;
                       for (let i = 0; i < botCount; i++) {
                           const sId = shipIds[i % 3];
                           const amount = Math.floor(Math.random() * 401) + 100;
                           const timeOffset = Math.floor(Math.random() * 17000); 
                           botActions.push({
                               id: `bot-${i}-${Date.now()}`, shipId: sId, amount, timeOffset,
                               color: BOT_COLORS[Math.floor(Math.random() * BOT_COLORS.length)],
                               initial: BOT_INITIALS[Math.floor(Math.random() * BOT_INITIALS.length)]
                           });
                           botPools[sId] += amount;
                       }
                       shuffleArray(botActions);
                       updates.botActions = botActions;
                       updates.botPools = botPools;
                       updates.botCount = botCount;
                    }
                }

                const newState = { ...state, ...updates, phase: nextPhase, endTime: now + duration, lastUpdate: now };
                transaction.set(stateRef, newState);
                return { state: newState, changed: true };
            });

            // IMPORTANT: If 'force' is true, we push to RTDB EVEN IF firestore didn't change phases.
            // This ensures broadcasts are restored if RTDB was wiped.
            if (result.changed || force) {
                const newState = result.state;
                await rtdb.ref('norns_fate/state').set(newState);

                if (result.changed) {
                    if (newState.phase === 'betting') {
                        await rtdb.ref('norns_fate/pools').set({ odin: newState.houseSeed, thor: newState.houseSeed, loki: newState.houseSeed });
                        await rtdb.ref('norns_fate/bettors').remove();
                        await clearBets(db.collection('settings').doc('mini_games').collection('norns_fate').doc('current'));
                    } else if (newState.phase === 'reveal') {
                         try {
                            const statePath = db.collection('settings').doc('mini_games').collection('norns_fate').doc('current');
                            await new Promise(resolve => setTimeout(resolve, 3000));
                            const betsSnap = await statePath.collection('bets').get();
                            const flushedPools: Record<string, number> = { odin: newState.houseSeed, thor: newState.houseSeed, loki: newState.houseSeed };
                            if (!betsSnap.empty) {
                                betsSnap.docs.forEach((doc: any) => {
                                    const betData = doc.data();
                                    ['odin','thor','loki'].forEach(id => {
                                         if (betData[id]) flushedPools[id] += betData[id];
                                    });
                                });
                            }
                            await rtdb.ref('norns_fate/pools').set(flushedPools);
                        } catch (e) {
                            console.error('Master Sync failed', e);
                        }
                    } else if (newState.phase === 'result') {
                        await processNornsFatePayouts(newState);
                    }
                }
            }
            return { success: true, state: result.state };
...
async function processNornsFatePayouts(state: any) {
    const db = admin.firestore();
    const rtdb = admin.database();
    const stateRef = db.collection('settings').doc('mini_games').collection('norns_fate').doc('current');
    
    const winnerId = state.winnerHand;
    if (!winnerId) return;

    const houseSeed = state.houseSeed ?? DEFAULT_HOUSE_SEED;
    const multiplierFactor = state.multiplierFactor ?? (1 - DEFAULT_HOUSE_CUT);
    const bets = await stateRef.collection('bets').get();
    ...
    if (state.botPools) {
        ['odin', 'thor', 'loki'].forEach(id => {
            totalPool += (state.botPools[id] || 0);
            if (id === winnerId) winnerPool += (state.botPools[id] || 0);
        });
    }
        ...
        for (const bet of betDocs) {
            ...
            try {
                await db.runTransaction(async (t: any) => {
                    ...
                    if (!userSnap.exists) return;
                    const userData = userSnap.data();
                    const clampedPoints = clampPointsToTierMax((userData.points || 0) + winAmount, userData.tier || 1);
                    t.update(userRef, { points: clampedPoints, exp: admin.firestore.FieldValue.increment(winAmount) });
                });
                ...
                console.error('Norns Payout Error:', e);
            }
        }
    }
}
