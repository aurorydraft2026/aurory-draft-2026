...
import { syncAuroryName, syncAuthPhoto } from '../services/auroryProfileService';
...
export const useAuth = (navigate) => {
    ...
    const handleSyncAuthPhoto = async () => {
        if (!user || !user.photoURL || isSyncing) return;
        setIsSyncing(true);
        try {
            const result = await syncAuthPhoto(user.uid, user.photoURL);
            if (result.success) {
                console.log('✅ Auth photo sync completed');
            } else {
                alert('Photo sync failed: ' + result.error);
            }
        } catch (error) {
            console.error('Photo sync error:', error);
            alert('An error occurred while syncing your photo.');
        } finally {
            setIsSyncing(false);
        }
    };
                ...
                >
                    ...
                    <div className="user-modal-content">
                        ...
                        <div className="user-header-info" style={getEquippedBannerStyle(user)}>
                            <div className="header-sync-actions">
                                {user.auroryPlayerId && (
                                    <button
                                ...
                                <div className="profile-checkin-row">
                                    ...
                                    >
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                            {isCheckedIn ? (
                                                <><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg><span className="checkin-btn-text">Checked In</span></>
                                            ) : (
                                                <><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg><span className="checkin-btn-text">Check In</span></>
                                            )}
                                        </div>
                                    </button>
                                    ...
                                    {isCheckedIn && (
                                        <div className="checkin-timer-wrapper" title="Time until next reward reset">
